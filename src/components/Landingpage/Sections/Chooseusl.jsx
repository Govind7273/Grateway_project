  ...
  return (
    <section>
      <motion.div
        ...
        initial="hidden"
        whileInView={"show"}
        viewport={{ once: true, amount: 0.8 }} className='text-center p-10'>
        <h1 className="m-4 text-3xl  text-center font-extrabold text-gray-900 dark:text-white md:text-4xl lg:text-4xl"> Why <span className="text-transparent bg-clip-text bg-[#6B37BD]">Choose Us</span></h1>
        <p className="text-lg font-normal text-center text-gray-600 lg:text-xl dark:text-gray-400">With a portfolio of successful projects across various industries, our proven track record speaks for itself. We consistently deliver high-quality software solutions on time and within budget. Trust us to bring your project to life with reliability and excellence.</p>
      </motion.div>
      <div className="w-[100vw] overflow-hidden bg-opacity-100  flex justify-center items-center">
        ...
        initial="hidden"
        whileInView={"show"}
        viewport={{ once: true, amount: 0.8 }} className="card bg-white cursor-pointer rounded-xl w-[300px] md:w-[400px] h-[230px] preserve-3d perspective bg-inherit text-center">
                {/* front face */}
                ...
                {/* back face */}
                <div className="back-face bg-cyan-400 px-[20px] py-[30px] text-center select-none absolute flex flex-col items-center justify-center w-[100%] h-[100%] overflow-hidden shadow-[0_15px_35px_rgba(0,_0,_0,_0.1)] rounded-[15px] preserve-3d transition-[0.5s] border-t-[1px_solid_#ddd] border-l-[1px_solid_#ddd] backface-invisible">
              ...
              initial="hidden"
              whileInView={"show"}
              viewport={{ once: true, amount: 0.8 }} className=" cursor-pointer card bg-white rounded-xl w-[300px] md:w-[400px] h-[230px] preserve-3d perspective bg-inherit text-center">
                {/* front face */}
                <div className="front-face absolute text-[#fff] w-[100%] h-[100%] overflow-hidden shadow-[0_15px_35px_rgba(0,_0,_0,_0.1)] rounded-[15px] bg-[rgba(255,_255,_255,_0.06)] preserve-3d transition-[0.5s] border-t-[1px_solid_#ddd] border-l-[1px_solid_#ddd] backface-invisible flex flex-col items-center justify-center ">
                  <div className="text-black flex flex-col items-center">
        ...
        <div className="text-Para flex items-center justify-center font-semibold text-gray-600 border-y border-gray-400">
          <ul className="flex flex-col w-fit md:flex-row justify-center gap-4">
            {Object.entries(industryMapping).map(([displayName, key]) => (
              <li key={key}>
                <div
                  className={`p-4 w-fit cursor-pointer border-b-2 border-transparent rounded-t-lg ${selectedIndustry === key ? 'text-white bg-[#6B37BD]' : ''}`}
          ...
          {selectedIndustry && (
            <ul className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              {industryData[selectedIndustry].map((item, index) => (
                ...
                initial="hidden"
                whileInView={"show"}
                viewport={{ once: true, amount: 0.8 }} key={index} className="flex items-start mx-4">
